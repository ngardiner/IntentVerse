...
def test_wildcard_permissions(session: Session):
    ...
    fs_role = session.exec(
        select(Role).where(Role.name == "filesystem_manager")
    ).first()
...
def test_rbac_api_endpoints(client: TestClient):
    ...
    service_headers = {
        "X-API-Key": os.environ.get("SERVICE_API_KEY", "test-service-key-12345")
    }
    ...
    create_response = client.post(
        "/users/", json=admin_user_data, headers=service_headers
    )
    assert create_response.status_code == 200

    # Login to get token
    login_response = client.post(
        "/auth/login", data={"username": "admin_api", "password": "adminpass"}
    )
    ...
    new_role_data = {"name": "test_role", "description": "A test role"}
...
def test_permission_denied_for_regular_user(client: TestClient):
    ...
    # Login to get token
    login_response = client.post(
        "/auth/login", data={"username": "regular_user", "password": "userpass"}
    )
    assert login_response.status_code == 200
    token = login_response.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
    ...
    new_user_data = {"username": "another_user", "password": "password123"}
...
def test_user_can_view_own_details(client: TestClient):
    ...
    # Login to get token
    login_response = client.post(
        "/auth/login", data={"username": "self_view_user", "password": "userpass"}
    )
    assert login_response.status_code == 200
    token = login_response.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}
