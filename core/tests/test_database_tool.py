...
class TestDatabaseTool:
    ...
    def test_query_valid_select_statement(self, database_tool_with_data, state_manager):
        ...
        expected_result = {
            ...
            "rows": (
                [[row.get(col) for col in result[0].keys()] for row in result]
                if result
                else []
            ),
        }
    ...
    def test_insert_data_method(self, database_tool):
        ...
        database_tool.execute_sql(
            "CREATE TABLE insert_test (id INTEGER, name TEXT, age INTEGER)"
        )
    ...
    def test_update_data_method(self, database_tool):
        ...
        # Update
        result = database_tool.update_data(
            "update_test", {"status": "completed"}, "id = ?", (1,)
        )
    ...
    def test_export_database_content(self, database_tool_with_data):
        ...
        create_statements = [
            sql for sql in exported_sql if sql.upper().startswith("CREATE")
        ]
        assert len(create_statements) >= 2  # products and users tables

        # Should have INSERT statements
        insert_statements = [
            sql for sql in exported_sql if sql.upper().startswith("INSERT")
        ]
    ...
    def test_multiple_queries_update_state(
        self, database_tool_with_data, state_manager
    ):
        ...
        expected_result1 = {
            ...
            "rows": (
                [[row.get(col) for col in result1[0].keys()] for row in result1]
                if result1
                else []
            ),
        }
        ...
        expected_result2 = {
            ...
            "rows": (
                [[row.get(col) for col in result2[0].keys()] for row in result2]
                if result2
                else []
            ),
        }
