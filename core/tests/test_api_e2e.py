...
def test_write_and_read_end_to_end():
    ...
    try:
        ...
        if execute_response.status_code == 401:
            pytest.skip(
                f"Authentication failed. API key may be incorrect. Response: {execute_response.text}"
            )

        # ASSERT (Step 1): Check if the write command was accepted
        assert (
            execute_response.status_code == 200
        ), f"Write request failed with status {execute_response.status_code}. Response: {execute_response.text}"
        response_data = execute_response.json()
        assert (
            response_data["status"] == "success"
        ), f"Write operation failed: {response_data}"
        ...
        ), f"State request failed with status {state_response.status_code}. Response: {state_response.text}"
        fs_state = state_response.json()

        # Find the newly created file in the root directory's children
        children = fs_state.get("children", [])
        found_file = next(
            (f for f in children if f.get("name") == "e2e-test.txt"), None
        )

        assert (
            found_file is not None
        ), f"The newly written file was not found in the state. Children: {children}"
        assert (
            found_file.get("type") == "file"
        ), f"Expected file type, got: {found_file.get('type')}"
...
async def test_execute_non_existent_tool_returns_404():
    ...
    payload = {"tool_name": "nonexistent.tool", "parameters": {}}
    ...
    try:
        ...
        if response.status_code == 401:
            pytest.skip(
                f"Authentication failed. API key may be incorrect. Response: {response.text}"
            )
        ...
        assert (
            "detail" in error_details
        ), f"Expected 'detail' in response: {error_details}"
        assert (
            "Tool 'nonexistent.tool' not found" in error_details["detail"]
        ), f"Unexpected error message: {error_details['detail']}"
    ...
    try:
        async with httpx.AsyncClient(headers=headers) as client:
            ...
            set_response = await client.post(
                f"{CORE_API_URL}/api/v1/execute", json=set_payload
            )

            # Check for authentication issues
            if set_response.status_code == 401:
                pytest.skip(
                    f"Authentication failed. API key may be incorrect. Response: {set_response.text}"
                )

            # Assert: Ensure the call was successful.
            assert (
                set_response.status_code == 200
            ), f"Set memory failed with status {set_response.status_code}. Response: {set_response.text}"
            set_result = set_response.json()
            assert (
                set_result.get("status") == "success"
            ), f"Set memory operation failed: {set_result}"
            ...
            get_response = await client.post(
                f"{CORE_API_URL}/api/v1/execute", json=get_payload
            )

            # Assert: Ensure the call was successful and returned the correct value.
            assert (
                get_response.status_code == 200
            ), f"Get memory failed with status {get_response.status_code}. Response: {get_response.text}"
            get_result = get_response.json()
            assert (
                "result" in get_result
            ), f"Expected 'result' in response: {get_result}"
            assert (
                get_result["result"] == test_value
            ), f"Expected '{test_value}', got '{get_result['result']}'"
    ...
    try:
        async with httpx.AsyncClient(base_url=CORE_API_URL, headers=headers) as client:
            ...
            if create_response.status_code == 401:
                pytest.skip(
                    f"Authentication failed. API key may be incorrect. Response: {create_response.text}"
                )

            assert (
                create_response.status_code == 200
            ), f"Failed to create table. Response: {create_response.text}"
            create_result = create_response.json()
            assert (
                create_result["status"] == "success"
            ), f"Create table operation failed: {create_result}"
            ...
            ), f"Failed to insert data. Response: {insert_response.text}"
            insert_result = insert_response.json()
            assert (
                insert_result["status"] == "success"
            ), f"Insert operation failed: {insert_result}"
            ...
            ), f"Failed to query data. Response: {query_response.text}"

            query_result = query_response.json()
            assert (
                query_result["status"] == "success"
            ), f"Query operation failed: {query_result}"
