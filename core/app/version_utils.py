...
def is_version_compatible(app_version: str, min_version: str, max_version: Optional[str] = None) -> bool:
    """
...
def check_compatibility_conditions(app_version: str, compatibility_conditions: List[dict]) -> Tuple[bool, List[str]]:
    """
    ...
    for condition in compatibility_conditions:
        ...
        if condition_type == "version_range":
            ...
            if not min_version:
                failure_reasons.append("Invalid compatibility condition: missing min_version")
                continue
            
            try:
                if not is_version_compatible(app_version, min_version, max_version):
                    if max_version:
                        failure_reasons.append(f"{reason} (requires {min_version} <= version <= {max_version}, got {app_version})")
                    else:
                        failure_reasons.append(f"{reason} (requires version >= {min_version}, got {app_version})")
            except ValueError as e:
                failure_reasons.append(f"Invalid version in compatibility condition: {e}")
        else:
            failure_reasons.append(f"Unknown compatibility condition type: {condition_type}")
