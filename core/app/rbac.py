...
class PermissionChecker:
    ...
    def get_user_permissions(self, user: User) -> Set[str]:
        ...
        if user.is_admin:
            ...
            if os.getenv("SERVICE_API_KEY") == "test-service-key-12345":
                logger.debug(
                    f"Added admin.all permission for admin user {user.username}"
                )
        ...
        if os.getenv("SERVICE_API_KEY") == "test-service-key-12345":
            logger.debug(
                f"Final permissions for user {user.username}: {sorted(list(permissions))}"
            )
    ...
    def has_permission(self, user: User, permission: str) -> bool:
        ...
        if os.getenv("SERVICE_API_KEY") == "test-service-key-12345":
            logger.debug(
                f"Checking permission '{permission}' for user {user.username} (is_admin={user.is_admin})"
            )
        ...
        for perm in user_permissions:
            if perm.endswith(".*"):
                ...
                if permission.startswith(prefix + "."):
                    if os.getenv("SERVICE_API_KEY") == "test-service-key-12345":
                        logger.debug(
                            f"Permission granted: wildcard '{perm}' grants '{permission}'"
                        )
                    return True

        if os.getenv("SERVICE_API_KEY") == "test-service-key-12345":
            logger.debug(
                f"Permission denied: no matching permission for '{permission}'"
            )
        return False
...
def require_permission_or_service(permission: str):
    ...
    def permission_or_service_dependency(
        current_user_or_service: Annotated[
            Union[User, str], Depends(_get_current_user_or_service())
        ],
        checker: Annotated[PermissionChecker, Depends(get_permission_checker)],
    ) -> Union[User, str]:
...
DEFAULT_ROLES = [
    # System roles
    ("admin", "System Administrator", True, ["admin.all"]),  # Grants everything
    (
        "user",
        "Standard User",
        True,
...
def create_default_roles(session: Session) -> None:
    ...
    for role_name, description, is_system, permission_names in DEFAULT_ROLES:
        ...
        if not existing_role:
            role = Role(
                name=role_name, description=description, is_system_role=is_system
            )
            session.add(role)
            session.commit()
