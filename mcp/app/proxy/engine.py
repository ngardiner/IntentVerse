...
class MCPProxyEngine:
    ...
    async def start(self) -> None:
        ...
        if (
            self.config is None
            or self.discovery_service is None
            or self.proxy_generator is None
        ):
        ...
        try:
            ...
            if self.config.global_settings.discovery_interval > 0:
                self._discovery_task = asyncio.create_task(
                    self._periodic_discovery_loop()
                )

            logger.info("MCP Proxy Engine started successfully")
            log_engine_event(
                "startup",
                "MCP Proxy Engine Started",
    ...
    async def refresh_tools(self, force_discovery: bool = False) -> DiscoveryResult:
        ...
        try:
            ...
            total_tools = sum(
                r.tools_discovered for r in discovery_results if r.success
            )
            ...
            combined_result = DiscoveryResult(
                ...
                discovery_time=max(
                    (r.discovery_time for r in discovery_results), default=0.0
                ),
            )

            logger.info(
                f"Tool refresh completed: {total_tools} tools from {successful_servers} servers"
            )
    ...
    async def _discover_and_generate_tools(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Discovery completed: {total_tools} tools from {successful_servers} servers"
            )
                    ...
                    for tool_name, proxy_func in proxy_functions.items():
                        if tool_name not in self._registered_tools:
                            try:
                                ...
                                logger.error(
                                    f"Failed to register new tool {tool_name}: {e}"
                                )
    ...
    def __repr__(self) -> str:
        return (
            f"MCPProxyEngine(running={self._running}, "
...
async def create_and_start_proxy_engine(
