...
class ToolRegistry:
    ...
    tools_by_server: Dict[str, List[MCPTool]] = field(default_factory=lambda: defaultdict(list))
    ...
    def _handle_conflict(self, tool_name: str, new_tool: MCPTool, existing_tool: MCPTool) -> None:
        ...
        logger.warning(f"Tool name conflict detected: {tool_name} exists in {conflict.servers}")
...
class ToolDiscoveryService:
    ...
    async def stop(self) -> None:
        ...
        if self._discovery_tasks:
            await asyncio.gather(*self._discovery_tasks.values(), return_exceptions=True)
    ...
    async def discover_all_tools(self, force_refresh: bool = False) -> List[DiscoveryResult]:
        """
        ...
        logger.info(f"Tool discovery completed: {successful}/{len(discovery_results)} servers, {total_tools} tools total")
    ...
    async def _discover_server_tools(self, server_name: str, client: MCPClient, 
        ...
        try:
            ...
            if removed_count > 0:
                logger.debug(f"Removed {removed_count} existing tools from {server_name}")
    ...
    async def discover_server_tools(self, server_name: str, force_refresh: bool = False) -> DiscoveryResult:
        """
                ...
                for server_name, client in self.clients.items():
                    try:
                        if not await client.is_healthy():
                            logger.warning(f"Server {server_name} is unhealthy, attempting reconnection")
                            ...
                            if await client.reconnect():
                                ...
                                if removed > 0:
                                    logger.info(f"Removed {removed} tools from unhealthy server {server_name}")
    ...
    def __repr__(self) -> str:
        return (f"ToolDiscoveryService(servers={list(self.clients.keys())}, "
