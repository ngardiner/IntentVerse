...
class TestMCPModeIntegration:
    ...
    async def verify_core_service(self, core_service_url):
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=timeout) as client:
                    ...
                    if response.status_code == 200:
                        print(
                            f"Successfully connected to Core service on attempt {attempt + 1}"
                        )
                        return core_service_url
            except (httpx.ConnectError, httpx.TimeoutException) as e:
                print(
                    f"Attempt {attempt + 1}/{max_retries}: Connection failed - {type(e).__name__}: {e}"
                )
            except Exception as e:
                print(
                    f"Attempt {attempt + 1}/{max_retries}: Unexpected error - {type(e).__name__}: {e}"
                )
        ...
        try:
            ...
            host = (
                core_service_url.replace("http://", "")
                .replace("https://", "")
                .split(":")[0]
            )
            port = (
                int(core_service_url.split(":")[-1])
                if ":" in core_service_url.split("//")[-1]
                else 80
            )
        ...
        pytest.skip(
            f"Core service not available at {core_service_url} after {max_retries} attempts"
        )
    ...
    @pytest.mark.asyncio
    async def test_http_mode_startup_and_health(
        self, verify_core_service, test_environment, mcp_app_path
    ):
        ...
        try:
                ...
                try:
                    async with httpx.AsyncClient() as client:
                        response = await client.get(
                            "http://localhost:8001/", timeout=5.0
                        )
                        if response.status_code == 200:
                            break
                except Exception:
                    if attempt == max_retries - 1:
                        ...
                        pytest.fail(
                            f"HTTP mode failed to start. STDOUT: {stdout}, STDERR: {stderr}"
                        )
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    "http://localhost:8001/", json=mcp_request, timeout=10.0
                )
    ...
    @pytest.mark.asyncio
    async def test_stdio_mode_startup_and_communication(
        self, verify_core_service, test_environment, mcp_app_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_both_modes_same_tools(
        self, verify_core_service, test_environment, mcp_app_path
    ):
        ...
        ), f"Tool sets differ. HTTP: {http_tools}, Stdio: {stdio_tools}"
        ...
        assert expected_tools.issubset(
            http_tools
        ), f"Missing expected tools: {expected_tools - http_tools}"

    @pytest.mark.asyncio
    async def test_stdio_mode_tool_execution(
        self, verify_core_service, test_environment, mcp_app_path
    ):
        ...
        try:
            ...
            assert (
                "stored" in content["text"].lower()
                or "success" in content["text"].lower()
            )
    ...
    @pytest.mark.asyncio
    async def test_http_mode_tool_execution(
        self, verify_core_service, test_environment, mcp_app_path
    ):
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                response = await client.post(
                    "http://localhost:8001/", json=mcp_request, timeout=15.0
                )
    ...
    @pytest.mark.asyncio
    async def test_core_client_connection_both_modes(
        self, verify_core_service, test_environment
    ):
        ...
        try:
            ...
            assert (
                "success" in str(response).lower() or "stored" in str(response).lower()
            )
    ...
    @pytest.mark.asyncio
    async def test_timeline_logging_both_modes(
        self, verify_core_service, test_environment, mcp_app_path
    ):
        ...
        try:
            ...
            assert (
                stdio_process.returncode == 0
            ), f"Timeline logging failed in stdio mode. STDERR: {stderr}"
...
class TestMCPModeErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_stdio_mode_core_service_unavailable(
        self, test_environment, mcp_app_path
    ):
