...
class TestProxyEngineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_proxy_engine_lifecycle(
        self, temp_config_file, mock_server_tools
    ):
        ...
        def mock_client_factory(server_config):
            ...
            mock_client.get_server_info = Mock(
                return_value=MCPServerInfo(
                    name=server_config.name,
                    version="1.0",
                    protocol_version="1.0",
        ...
        with patch("app.proxy.discovery.MCPClient", side_effect=mock_client_factory), \
            ...
            ), f"Expected tool {expected_tool_name} not found in {list(proxy_functions.keys())}"
    ...
    async def test_proxy_engine_with_tool_conflicts(self, temp_config_file):
        ...
        try:
            ...
            with patch(
                "app.proxy.discovery.MCPClient", side_effect=mock_client_factory
            ):
                await engine.initialize()
                await engine.start()
                ...
                # Debug: Print all available proxy functions
                print(
                    f"DEBUG: Available proxy functions for conflict test: {list(proxy_functions.keys())}"
                )
                ...
                ), f"Expected tool {expected_tool1} not found in {list(proxy_functions.keys())}"
                assert (
                    expected_tool2 in proxy_functions
                ), f"Expected tool {expected_tool2} not found in {list(proxy_functions.keys())}"
        ...
        try:
            ...
            def mock_client_factory(server_config):
                ...
                mock_client.call_tool = AsyncMock(
                    side_effect=mock_enabled_server.call_tool
                )
                mock_client.is_healthy = AsyncMock(return_value=True)
                mock_client.get_server_info = Mock(
                    return_value=MCPServerInfo(
    ...
    async def test_proxy_engine_connection_failure(self, temp_config_file):
        ...
        with patch(
            "app.proxy.discovery.MCPClient", side_effect=mock_failing_client_factory
        ):
            await engine.initialize()
            await engine.start()
    ...
    @pytest.mark.asyncio
    async def test_create_and_start_proxy_engine_integration(
        self, temp_config_file, mock_server_tools
    ):
    ...
    async def test_proxy_tool_parameter_validation_integration(self, temp_config_file):
        ...
        with patch("app.proxy.discovery.MCPClient", side_effect=mock_client_factory):
            ...
            # Debug: Print all available proxy functions
            print(
                f"DEBUG: Available proxy functions for strict validation test: {list(proxy_functions.keys())}"
            )
