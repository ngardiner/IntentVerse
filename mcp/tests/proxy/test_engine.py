...
class TestMCPProxyEngine:
    ...
    async def test_initialize_config_error(self, engine):
        ...
        with patch(
            "app.proxy.engine.load_proxy_config", side_effect=Exception("Config error")
        ):
    ...
    async def test_start_engine(self, engine, mock_config):
        ...
        with (
            patch.object(engine.discovery_service, "start") as mock_discovery_start,
    ...
    async def test_register_proxy_tools(self, engine, mock_config):
        ...
        mock_functions = {"test_tool1": Mock(), "test_tool2": Mock()}
        engine.proxy_generator.get_all_proxy_functions = Mock(
...
class TestCreateAndStartProxyEngine:
    ...
    async def test_create_and_start_success(self):
        ...
        with (
            patch("app.proxy.engine.load_proxy_config", return_value=mock_config),
    ...
    async def test_create_and_start_with_config_path(self):
        ...
        with (
            patch(
                "app.proxy.engine.load_proxy_config", return_value=mock_config
            ) as mock_load,
            patch.object(MCPProxyEngine, "initialize") as mock_init,
            patch.object(MCPProxyEngine, "start") as mock_start,
        ):
    ...
    async def test_create_and_start_initialization_error(self):
        ...
        with patch.object(
            MCPProxyEngine, "initialize", side_effect=Exception("Init failed")
        ):
    ...
    async def test_create_and_start_start_error(self):
        ...
        with (
            patch.object(MCPProxyEngine, "initialize") as mock_init,
            patch.object(
                MCPProxyEngine, "start", side_effect=Exception("Start failed")
            ),
        ):
