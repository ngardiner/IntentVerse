...
class TestRegistrarUnit:
    ...
    def test_create_dynamic_proxy_simple(self, registrar_instance):
        ...
        tool_def = {
            ...
            "parameters": [{"name": "param1", "annotation": "str", "required": True}],
        }
    ...
    def test_create_dynamic_proxy_with_optional_param(self, registrar_instance):
        ...
        assert (
            sig.parameters["optional_param"].default is None
    ...
    async def test_proxy_function_execution(self, registrar_instance):
        ...
        registrar_instance.core_client.execute_tool = AsyncMock(
        ...
        registrar_instance.core_client.execute_tool.assert_called_once_with(
            {"tool_name": "test.simple", "parameters": {"param1": "test_value"}}
        )
...
class TestMCPIntegration:
    ...
    async def test_tool_registration(self, registrar, mcp_server, sample_tool_manifest):
        ...
        for tool_name in expected_tool_names:
            ...
            assert (
                retrieved_tool is not None
            ), f"Tool {tool_name} should be retrievable from server"
    ...
    async def test_mcp_request_handling(self, sample_tool_manifest):
        ...
        execute_route = respx.post(execute_url).mock(
            return_value=httpx.Response(
                200, json={"status": "success", "result": "mocked result"}
            )
        )
        ...
        tool_def = next(
            tool
            for tool in sample_tool_manifest
